...
contract Deploy is Broadcaster {
    function run() public returns (GaugeFactoryRootstockCollective) {
        ...
        return run(_beaconAddress, _rifTokenAddress);
    }

    function run(address beacon_, address rifToken_) public broadcast returns (GaugeFactoryRootstockCollective) {
        ...
        if (vm.envOr("NO_DD", false)) {
            return new GaugeFactoryRootstockCollective(beacon_, rifToken_);
        }
        return new GaugeFactoryRootstockCollective{ salt: _salt }(beacon_, rifToken_);
    }
}
