...
import { Deploy as CycleTimeKeeperRootstockCollectiveDeployer } from "script/CycleTimeKeeperRootstockCollective.s.sol";
  ...
  function run() public {
    (
      ...
      GovernanceManagerRootstockCollective _governanceManagerImpl
        ...
        _governorAddress,
        _foundationTreasuryAddress,
        _kycApproverAddress,
        _upgrader
      );
    saveWithProxy(
      "GovernanceManagerRootstockCollective",
      address(_governanceManagerImpl),
      address(_governanceManagerProxy)
    );

    GaugeBeaconRootstockCollective _gaugeBeacon = new GaugeBeaconRootstockCollectiveDeployer().run(
      address(_governanceManagerProxy)
    );
    ...
    (
      CycleTimeKeeperRootstockCollective _timeKeeperProxy,
      CycleTimeKeeperRootstockCollective _timeKeeperImpl
    ) = new CycleTimeKeeperRootstockCollectiveDeployer().run(
        address(_governanceManagerProxy),
        _cycleDuration,
        _cycleStartOffset,
        _distributionDuration
      );
    saveWithProxy("CycleTimeKeeperRootstockCollective", address(_timeKeeperImpl), address(_timeKeeperProxy));
