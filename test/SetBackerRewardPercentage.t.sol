...
contract SetBackerRewardPercentageTest is BaseTest {
    ...
    //     vm.expectEmit();
    //     emit BackerRewardPercentageUpdateScheduled(activeBuilder, 0.1 ether, block.timestamp + 2 weeks);
    //     builderRegistry.setBackerRewardPercentage(0.1 ether);
    ...
    //     assertEq(builderRegistry.getRewardPercentageToApply(activeBuilder), 0.5 ether);
    // }
    // /**
    ...
    //     (uint64 _previous, uint64 _next, ) = builderRegistry.backerRewardPercentage(activeBuilder);
    ...
    //     builderRegistry.pauseBuilder(activeBuilder, "paused");
    ...
    //     (_previous, _next, _cooldownEndTime) = builderRegistry.backerRewardPercentage(activeBuilder);
    //     // THEN previous backer reward percentage is 50%
    //     assertEq(_previous, 0.5 ether);
    ...
    //     assertEq(_clearERC20Balance(activeBuilder), 6.25 ether);
    //     // THEN builder receives 50% of coinbase 0.625 = (10 * 2 / 16) * 0.5
    //     assertEq(_clearCoinbaseBalance(activeBuilder), 0.625 ether);
    ...
    //     assertEq(builderRegistry.getRewardPercentageToApply(activeBuilder), 0.8 ether);
    //     // AND 100 rewardToken and 10 coinbase are distributed
    //     _distribute(100 ether, 10 ether);
    //     // WHEN builder claim rewards
    //     _buildersClaim();
    //     // THEN builder receives 20% of rewards 2.5 = (100 * 2 / 16) * 0.2
    //     assertEq(_clearERC20Balance(activeBuilder), 2.5 ether);
    //     // THEN builder receives 20% of coinbase 0.25 = (10 * 2 / 16) * 0.2
    //     assertEq(_clearCoinbaseBalance(activeBuilder), 0.25 ether);
    //     // WHEN alice claims the rewards
    //     _skipAndStartNewCycle();
    //     vm.prank(alice);
    //     gauge.claimBackerReward(alice);
    ...
    //     assertEq(builderRegistry.getRewardPercentageToApply(activeBuilder), 0.1 ether);
    //     // AND builder sets the reward percentage to 80%
    //     vm.prank(activeBuilder);
    //     builderRegistry.setBackerRewardPercentage(0.8 ether);
    ...
    //     assertEq(_clearERC20Balance(activeBuilder), 11.25 ether);
    //     // THEN builder receives 90% of coinbase 1.125 = (10 * 2 / 16) * 0.9
    //     assertEq(_clearCoinbaseBalance(activeBuilder), 1.125 ether);
    //     // WHEN alice claims the rewards
    //     _skipAndStartNewCycle();
    //     vm.prank(alice);
    //     gauge.claimBackerReward(alice);
    ...
    //     _initialState();
    //     // AND cooldown time ends
    //     (, , uint128 _cooldownEndTime) = builderRegistry.backerRewardPercentage(activeBuilder);
    //     vm.warp(_cooldownEndTime);
    //     // THEN backer reward percentage to apply is 10%
    //     assertEq(builderRegistry.getRewardPercentageToApply(activeBuilder), 0.1 ether);
    //     // AND 100 rewardToken and 10 coinbase are distributed
