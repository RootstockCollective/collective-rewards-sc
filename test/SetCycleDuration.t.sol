...
contract SetCycleDurationTest is BaseTest {
  ...
  function test_OnlyValidChangerOrFoundation() public {
    ...
    timeKeeper.setCycleDuration(3 weeks, 0 days);
    ...
    timeKeeper.setCycleDuration(3 weeks, 0 days);
    vm.prank(governor);
    timeKeeper.setCycleDuration(2 weeks, 0 days);
  }
  ...
  function test_RevertCycleDurationTooShort() public {
    ...
    timeKeeper.setCycleDuration(distributionDuration, 0 days);
  }
  ...
  function test_SetCycleDuration() public {
    ...
    //   THEN NewCycleDurationScheduled event is emitted
    (uint32 _previousDuration, uint32 _nextDuration, uint64 _previousStart, uint64 _nextStart, ) = timeKeeper
      .cycleData();
    ...
    (_previousDuration, _nextDuration, _previousStart, _nextStart, ) = timeKeeper.cycleData();
    ...
    (uint256 _cycleStart, uint256 _cycleDuration) = timeKeeper.getCycleStartAndDuration();
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + 1 weeks);
    ...
    vm.prank(governor);
    timeKeeper.setCycleDuration(6 weeks, 0 days);
    (_previousDuration, _nextDuration, _previousStart, _nextStart, ) = timeKeeper.cycleData();
    ...
    (_cycleStart, _cycleDuration) = timeKeeper.getCycleStartAndDuration();
    // THEN cycle start is now
    assertEq(_cycleStart, block.timestamp);
    // THEN cycle duration is 3 week
    assertEq(_cycleDuration, 3 weeks);
    // THEN cycle finishes in 3 week
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + 3 weeks);
  }
  ...
  function test_SetCycleDurationSameValue() public {
    ...
    vm.prank(governor);
    timeKeeper.setCycleDuration(1 weeks, 0 days);

    (uint32 _previousDuration, uint32 _nextDuration, , , ) = timeKeeper.cycleData();
  ...
  function test_SetCycleDurationTwiceBeforeCooldown() public {
    ...
    (uint32 _previousDuration, uint32 _nextDuration, , uint64 _nextStart, ) = timeKeeper.cycleData();
    // AND cycle didn't finish, 1 sec is remaining
    vm.warp(block.timestamp + 1 weeks - 1);

    // AND governor sets a new cycle duration of 4 weeks
    vm.prank(governor);
    timeKeeper.setCycleDuration(4 weeks, 0 days);
    (_previousDuration, _nextDuration, , _nextStart, ) = timeKeeper.cycleData();
    // THEN previous cycle duration is 1 week
    assertEq(_previousDuration, 1 weeks);
  ...
  function test_SetCycleDurationLonger() public {
    ...
    // THEN cycle starts now
    assertEq(timeKeeper.cycleStart(block.timestamp), block.timestamp);
    // THEN distribution window ends in 1 hour
    assertEq(timeKeeper.endDistributionWindow(block.timestamp), block.timestamp + 1 hours);
  ...
  function test_SetCycleDurationShorter() public {
    ...
    vm.prank(governor);
    timeKeeper.setCycleDuration(0.5 weeks, 0 days);
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + 0.5 weeks);
  ...
  function test_SameCycleDurationWithOffset() public {
    ...
    vm.prank(governor);
    timeKeeper.setCycleDuration(1 weeks, 3 days);
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + 1 weeks + 3 days);
  ...
  function test_LongerCycleDurationWithOffset() public {
    ...
    vm.prank(governor);
    timeKeeper.setCycleDuration(1.5 weeks, 3 days);
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + 1.5 weeks + 3 days);
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + 1.5 weeks);
  ...
  function test_ShorterCycleDurationWithOffset() public {
    ...
    vm.prank(governor);
    timeKeeper.setCycleDuration(0.75 weeks, 3 days);
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + 0.75 weeks + 3 days);
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + 0.75 weeks);
