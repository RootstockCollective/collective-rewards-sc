...
contract BackersManagerRootstockCollectiveTest is BaseTest {
  ...
  function test_RevertNotInDistributionPeriod() public {
    ...
    for (uint256 i = 0; i < 20; i++) {
      GaugeRootstockCollective _newGauge = _whitelistBuilder(makeAddr(string(abi.encode(i + 10))), builder, 1 ether);
  ...
  function test_InitializedWithAnCycleStartOffset() public {
    ...
    stdstore.target(address(timeKeeper)).sig("cycleData()").depth(4).enable_packed_slots().checked_write(_newOffset);
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + 8 weeks);
