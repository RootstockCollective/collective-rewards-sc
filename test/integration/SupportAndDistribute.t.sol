...
contract SupportAndDistributeTest is BaseTest {
    ...
    function test_integration_SupportAndDistribute() public {
        ...
        // AND 100 rifTokens and 10 native cpind are distributed
        ...
        // AND 100 rifTokens and 10 native tokens are distributed
        _distribute(100 ether, 10 ether);
        _skipAndStartNewCycle();
        ...
        // THEN builder 8 receives 0.36 rifToken = 60 * 2 / 333
        assertApproxEqAbs(_clearERC20Balance(builders[8]), 3_603_603_603_603_603_603, 100);
        // THEN builder 8 receives 0.036 native tokens = 6 * 2 / 333
        assertApproxEqAbs(_clearNativeBalance(builders[8]), 360_360_360_360_360_360, 100);
        ...
        // THEN builder 4 receives rifToken 0.32 = 60 * 2 / 366
        assertEq(_clearERC20Balance(builders[8]), 3_278_688_524_590_163_934);
        // THEN builder 4 receives native tokens 0.032 = 6 * 2 / 366
        assertEq(_clearNativeBalance(builders[8]), 327_868_852_459_016_394);

        // THEN builder 5 receives rifToken 32.78 = 60 * 200 / 366
        assertEq(_clearERC20Balance(builders[10]), 32_786_885_245_901_639_345);
        // THEN builder 5 receives native tokens 3.278 = 6 * 200 / 366
