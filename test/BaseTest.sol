...
contract BaseTest is Test {
  ...
  CycleTimeKeeperRootstockCollective public timeKeeperImpl;
  ...
  function setUp() public {
    ...
    (timeKeeper, timeKeeperImpl) = new CycleTimeKeeperRootstockCollectiveDeployer().run(
      address(governanceManager),
      cycleDuration,
      cycleStartOffset,
  ...
  function _skipAndStartNewCycle() internal {
    uint256 _currentCycleRemaining = timeKeeper.cycleNext(block.timestamp) - block.timestamp;
  ...
  function _skipToEndDistributionWindow() internal {
    _skipAndStartNewCycle();
    uint256 _currentCycleRemaining = timeKeeper.endDistributionWindow(block.timestamp) - block.timestamp;
    skip(_currentCycleRemaining);
  }
