...
contract SetCycleDurationFuzzTest is BaseFuzz {
  ...
  function testFuzz_SetCycleDuration(
    ...
    vm.prank(governor);
    timeKeeper.setCycleDuration(newCycleDuration_, cycleStartOffset_);
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + newCycleDuration_ + cycleStartOffset_);
    ...
    assertEq(timeKeeper.cycleNext(block.timestamp), block.timestamp + newCycleDuration_);
    ...
    for (uint256 i = 0; i < gaugesArray.length; i++) {
      assertApproxEqAbs(rewardToken.balanceOf(builders[i]), _calcBuilderReward(RT_DISTRIBUTION_AMOUNT * 3, i), 100);
    ...
    for (uint256 i = 0; i < backersArray.length; i++) {
      ...
      assertApproxEqAbs(backersArray[i].balance, _calcBackerReward(CB_DISTRIBUTION_AMOUNT * 3, i), 0.000000001 ether);
    }
