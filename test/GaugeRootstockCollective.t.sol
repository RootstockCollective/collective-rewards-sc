...
contract GaugeRootstockCollectiveTest is BaseTest {
  ...
  function test_onlyBackersManager() public {
    ...
    uint256 _timeUntilNextCycle = timeKeeper.timeUntilNextCycle(block.timestamp);
  ...
  function test_Allocate() public {
    ...
    assertEq(gauge.lastUpdateTime(address(rewardToken)), timeKeeper.cycleStart(block.timestamp));
  }
  ...
  function test_NotifyRewardAmountWithStrategy() public {
    ...
    assertEq(backersManager.periodFinish(), timeKeeper.cycleNext(block.timestamp));
  ...
  function test_ClaimBuilderRewards2Distributions() public {
    ...
    vm.warp(timeKeeper.endDistributionWindow(block.timestamp) - 1);
    rewardToken.mint(address(rewardDistributor), 100 ether);
  ...
  function _setPeriodFinish() internal {
    stdstore.target(address(backersManager)).sig("periodFinish()").checked_write(timeKeeper.cycleNext(block.timestamp));
  }
}
