...
abstract contract BuilderRegistryRootstockCollective is CycleTimeKeeperRootstockCollective, ERC165Upgradeable {
    ...
    /**
     * @notice Builder submites a request to replace his rewardReceiver address,
     * the request will then need to be appoved by `approveBuidlerRewardReceiverReplacement`
     * @dev reverts if Buider is not Operational
    ...
    function cancelRewardReceiverReplacementRequest() external {
        // By overriding the replacement with the current, it efectily cancels the request
     ...
     * @dev reverts is provided `rewardReceiverReplacement_` don't not matched Builder's request
    ...
    function _submitRewardReceiverReplacementRequest(address newRewardReceiver_) internal {
        ...
        // Only oprational builder can initiate this action
