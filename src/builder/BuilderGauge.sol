...
contract BuilderGauge {
    ...
    modifier onlySupportHub() {
        if (msg.sender != address(supportHub)) revert NotSupportHub();
        _;
    }
    ...
    function claimSupporterReward(address supporter_) external {
        ...
        if (reward > 0) {
            ...
            emit SupporterRewardsClaimed(supporter_, reward);
        }
    }
    ...
    /**
     * @notice called on the reward distribution. Transfers reward tokens from SupportHub to this contract
     * @dev reverts if caller si not the supportHub contract
