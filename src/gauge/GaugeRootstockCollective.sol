...
contract GaugeRootstockCollective is ReentrancyGuardUpgradeable {
   ...
   * @param builder_ address of the builder the gauge belongs to
   */
  function initialize(address rewardToken_, address backersManager_, address builder_) external initializer {
  ...
  function claimBuilderReward(address rewardToken_) public {
    address _rewardReceiver = backersManager.builderRewardReceiver(builder);
    if (backersManager.isBuilderPaused(builder)) revert BuilderRewardsLocked();
    if (msg.sender != builder && msg.sender != _rewardReceiver) revert NotAuthorized();
    ...
    if (msg.sender == _rewardReceiver && backersManager.hasBuilderRewardReceiverPendingApproval(builder)) {
  ...
  ) external payable onlyBackersManager returns (uint256 newGaugeRewardShares_) {
  ...
  function _rewardPerToken(address rewardToken_, uint256 periodFinish_) internal view returns (uint256) {
    ...
    // [PREC] = (([N] - [N]) * [PREC]) / [N]
    uint256 _rewardPerTokenCurrent = ((__lastTimeRewardApplicable - _lastUpdateTime) * _rewardData.rewardRate) /
      totalAllocation;
    // [PREC] = [PREC] + [PREC]
  ...
  function _notifyRewardAmount(
    ...
    _rewardRate = (backersAmount_ * UtilsLib._PRECISION + _rewardData.rewardMissing + _leftover) / timeUntilNextCycle_;
